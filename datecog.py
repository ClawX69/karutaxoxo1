...
enabled_channels = [894832773821517874]
ecolor = 0x00FFFF
...
async def rundate(img_url, msg, need_ring, uid):
    global cur_turn, initial_resource, respawn_matrix, cur_row, cur_col, lastrunstrings, lastrunlink, cddict, lastuser
    ...
    if startedmoving:
        await msg.channel.send(f"<@{uid}> you already moved, sorry the solution cannot be generated.")
        return False, "", ""
    ...
    if poterror:
        ...
        desc = ""
        rvembed = discord.Embed(title="Solution", description=solution, colour = ecolor)
        ...
        rvembed.add_field(name="Remaining Resource:", value=f"{remainingresource}\n\n[Click here](https://discord.gg/wA68vqK9Pw) **to join our discord server**", inline=False)
